...
SELECT ?mainCatName ?categoryName (COUNT(?category) AS ?categoryAmount) WHERE {
       ...
       const partTwoQuery = `> skos:narrower* ?continent .
...
export default async function getData(mainContinent){
    const response = await fetch(url+'?query='+ encodeURIComponent(partOneQuery + mainContinent + partTwoQuery) +'&format=json');
